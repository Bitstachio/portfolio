...
- **Statically Typed Icons**: An `index.ts` file maps the SVG components to a central `icons` object. By using `keyof typeof icons`, the available icon names are statically typed to prevent runtime errors.
